...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    if (list.elementAtOrNull(1) == null)
        return Pair(-1, -1)
    var counterFirstEl = 0
    var counterLastEl = 0
    var deepList = list.lastIndexOf(list.last())
    for (counterFirstEl in 0 until deepList) {
        for (counterLastEl in counterFirstEl + 1..deepList)
            if (list[counterFirstEl] + list[counterLastEl] == number)
                return Pair(counterFirstEl, counterLastEl)
    }
    return Pair(-1, -1)
}
 ...
 *   ) -> emptySet()
 */
// Решение с использованием информации из википедии!
